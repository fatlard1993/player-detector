#!/usr/bin/env python3
"""Generate Player Detector's mod menu icon: the lapis plate, sensing.
...
OUT = os.path.join(HERE, "src/main/resources/assets/player-detector-justfatlard/icon.png")
...
PULSE = (0xE3, 0x2A, 0x1E, 0xFF)


def build_icon():
    """The detector as it is built: a lapis top on a smooth stone plate, with a
    redstone pulse rising off it - the block's whole job in one look."""
    stone = [[shade(px, 0.8) for px in row] for row in vanilla("block/smooth_stone.png")]
    lapis = vanilla("block/lapis_block.png")
    icon = [row[:] for row in stone]
    for y in range(13, 16):
        icon[y] = lapis[y][:]
    # The pulse: three arcs widening as they rise
    for x in range(6, 10):
        icon[11][x] = PULSE
    for x, y in ((5, 9), (6, 8), (7, 8), (8, 8), (9, 8), (10, 9)):
        icon[y][x] = PULSE
    for x, y in ((3, 6), (4, 5), (5, 4), (6, 3), (7, 3), (8, 3), (9, 3), (10, 4), (11, 5), (12, 6)):
        icon[y][x] = PULSE
    return scale(icon, 8)
